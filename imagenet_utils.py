...
# from tensorpack.dataflow import (
#     AugmentImageComponent, BatchData, MultiThreadMapData,
#     MultiProcessRunner, dataset, imgaug)
...
class ImageNetModel(ModelDesc):
    ...
    """
    Confidence regularization, 0 disables it.
    """
    conf_regularization = 0.
    ...
    def build_graph(self, image, label):
        ...
        loss = ImageNetModel.compute_loss_and_error(
            ...
            conf_regularization=self.conf_regularization)
    ...
    @staticmethod
    def compute_loss_and_error(logits, label, label_smoothing=0., conf_regularization=0.):
        ...
        if conf_regularization > 0.:
            nclass = logits.get_shape().as_list()[-1]
            max_h = -tf.log(1.0 / nclass)
            prob = tf.nn.softmax(logits * conf_regularization)
            h = -tf.reduce_sum(prob * tf.log(tf.maximum(prob, 1e-08)), axis=-1)
            loss_c = tf.reduce_mean(tf.truediv(max_h - h, conf_regularization), name='confidence-loss')
            add_moving_summary(loss_c)
            loss = loss + loss_c
